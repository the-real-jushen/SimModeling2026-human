...
假设我们有一组带误差的观测点 $(x_i, y_i)$，并且猜测其遵循某种真实模型规律 $f(x, Beta)$（例如直线模型 $f(x) = eta_0 + eta_1 x$）。
模型在你给定的每一个测量点 $x_i$ 处，都会给出一个理论预测值 $\hat{y}_i = f(x_i, Beta)$。
此时，实验真实值 $y_i$ 与理论预测值 $\hat{y}_i$ 之间就产生了**残差（Residual）**：$e_i = y_i - \hat{y}_i$。

最小二乘法的核心思想就是：**寻找一组最佳参数组合 $Beta$，使得所有数据点的残差平方和（Sum of Squared Residuals, SSR）最小化**：
